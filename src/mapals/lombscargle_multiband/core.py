...
class LombScargleMultiband(LombScargle):
    ...
    def __init__(
        ...
        self._trel, self.y, self.bands, self.dy = self._validate_inputs(trel, y, bands, dy)
    ...
    def from_timeseries(
        ...
        **kwargs
        ...
        if signal_column is None:
            raise ValueError("signal_column_name should be set to a list of valid column names")

        if band_labels is not None:
            if len(band_labels) != len(signal_column):
                raise ValueError("band_labels have an equal number of elements to signal_column")
        else:
        ...
        if isinstance(timeseries, TimeSeries):
            ...
            raise TypeError("Input time series should be an instance of " "TimeSeries or BinnedTimeSeries")
    ...
    def autopower(
        ...
        power = self.power(frequency, method=method, sb_method=sb_method, normalization=normalization)

        return frequency, power

    def power(self, frequency, method="flexible", sb_method="auto", normalization="standard"):
    ...
    def design_matrix(self, frequency, t_fit=None, bands_fit=None):
        ...
        if t_fit is None:
            ...
            t_fit, dy = strip_units(self._validate_t(self._as_relative_time("t", t_fit)), None)
    ...
    def offset(self, t_fit=None, bands_fit=None):
        ...
        if np.shape(t_fit) != np.shape(bands_fit):  # No need to broadcast if bands map to timestamps
            t_fit, bands_fit = np.broadcast_arrays(t_fit, bands_fit)

        # need to make sure all unique filters are represented
        u, i = np.unique(np.concatenate([bands_fit.ravel(), unique_bands]), return_inverse=True)
