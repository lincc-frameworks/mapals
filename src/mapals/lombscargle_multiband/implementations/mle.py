...
def design_matrix(t, bands, frequency, dy=None, nterms_base=1, nterms_band=1):
    ...
    cols = [np.ones(len(t))]
    cols = sum(
        ([np.sin((i + 1) * omega * t), np.cos((i + 1) * omega * t)] for i in range(nterms_base)),
        cols,
    )
    ...
    for band in unique_bands:
        ...
        cols = sum(
            ([np.sin((i + 1) * omega * t), np.cos((i + 1) * omega * t)] for i in range(nterms_band)),
            cols,
        )
        mask = bands == band
...
def construct_regularization(bands, nterms_base=1, nterms_band=1, reg_base=None, reg_band=1e-6):
    unique_bands = np.unique(bands)
...
def periodic_fit(
    ...
    # Calculate ymeans
    ymeans = np.zeros(y.shape)  # An array of shape y, with each index given a filter specific mean
    if center_data:
        for band in unique_bands:
            mask = bands == band
    ...
    X = design_matrix(t, bands, frequency, dy=dy, nterms_base=nterms_base, nterms_band=nterms_band)
    M = np.dot(X.T, X)
    ...
    if regularization is not None:
        diag = M.ravel(order="K")[:: M.shape[0] + 1]  # M is being affected by operations on diag
